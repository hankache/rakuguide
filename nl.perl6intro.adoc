...
Een subroutine kan nul of meer *parameters* definieren.
Het aantal en type van de parameters wordt het *signatuur* van de subroutine genoemd.
...
=== Multiple dispatch
Het is mogelijk om meer dan een subroutine met dezelfde naam, maar met een andere signatuur te definieren.
Zodra men de subroutine aanroept, bepaalt de uitvoerder welke versie van de subroutine wordt aangeroepen, afhankelijk van het aantal en type van de gegeven argumenten.
Dit soort van subroutines definieert men verder hetzelfde als normale subroutines, echter men vervangt het sleutelwoord `sub` door `multi`.
...
Als een subroutine gedefinieerd is om een argument te accepteren en we roepen het aan zonder dit benodigde argument, dan zal de aanroep een foutmelding geven.

Anderszins geeft Perl 6 ons de mogelijkheid om subroutines te definieren met:

* Optional Parameters
* Default Waarden van Parameters

Optionele parameters worden gedefinieerd foor een `?` achter de naam van de parameter te plakken.

[source,perl6]
----
sub zeg-hallo($naam?) {
  with $name { say "Hallo " ~ $naam }
  else { say "Hallo Mens" }
}
zeg-hallo;
zeg-hallo("Laura");
...
Als de gebruiker geen argument aan de subroutine geeft, dan kan men voor een default waarde zorgen. +
Dit wordt gedaan door een waarde toe te kennen aan de parameter in de definitie van de subroutine.

[source,perl6]
----
sub zeg-hallo($naam="Matt") {
  say "Hello " ~ $name;
}
zeg-hallo;
zeg-hallo("Laura");
