...
Dit bekrachtigd het concept van inkapseling.  De attributen van een object kunnen alleen worden gemanipuleerd vanaf binnen de klasse van het object door gebruik van methoden.
...
=== Overerving
==== Inleiding
*Overerving* is een ander concept van object georienteerd programmeren.

Wanneer men klassen aan het definieren is, komt men er snel genoeg achter dat sommige attributen/methoden in vele klassen voorkomen. +
Moeten we dus maar code gaan dupliceren? +
NEE! We moeten gebruik maken van *overerving*

Laten we aannemen dat we twee klassen willen definieren, een klasse voor Mensen en een klasse voor Werknemers. +
Mensen hebben twee attributen: naam en leeftijd. +
Werknemers hebben 4 attributen: naam, leeftijd, bedrijf en salaris

Men zou geneigd kunnen zijn om de klassen als volgt te definieren:
[source,perl6]
----
class Mens {
    has $.naam;
...
In het taalgebruik van object georienteerd programmeren, zeggen we dat Werknemer een *kind* is van Mens, en dat Mens de *ouder* is van Werknemer.

Alle kinderklassen erven de attributen en methoden van de ouderklasse, zodat het niet nodig is om deze overnieuw te definieren.

==== Overnemen
Klassen erven alle attributen en methoden van hun ouderklas. +
Er zijn gevallen waarin we willen det de methode in een kinderklasse zich anders gedraagt als methode die geerfd is van de ouderklasse. +
Om dit te bereiken, definieren we die methode ook in de kinderklasse. +
To achieve this, we redefine the method in the child class. +
We noemen dit concept *overnemen* (overriding).

In het onderstaande voorbeeld wordt de method `stel-jezelf-voor` geerfd door de Werknemer klasse.
...
==== Submethoden
*Submethoden* zijn een soort methoden die niet worden geerfd door kinderklassen. +
...
Meervoudige overerving is toegestaan in Perl 6.  Een klasse kan van meer dan een andere klasse erven.
...
Slechts een reeks van waarden werd getoond. +
Waarom gebeurde dit? +
`combo-diagram` erft van `lijndiagram` en `staafdiagram` en beide hebben ze een methode die `plot` heet.
Indien we die methode op `combo-diagram` aanroepen zal Perl 6 dit conflict proberen op te lossen door een van de geerfde methoden aan te roepen.

.Correctie
Om ervoor te zorgen dat dit correct functioneert, hadden we een methode `plot` in de `combo-diagram` klasse moeten definieren.
...
=== Rollen
*Rollen* zijn net als klassen in de zin van dat zij een verzamelijk van attributen en methoden zijn.
...
degene waarin we hadden _vergeten_ om de `plot` methode te definieren.
...
.Uitleg
Indien een klasse meer dan een rol doet en er daarbij een conflict optreedt, dan zal de compiler een foutmelding geven. +
Dit is een veel veiligere aanpak van meervoudige overerving waarbij conflicten niet als een probleem worden gezien en er bij de uitvoering zo maar iets gedaan wordt.
...
Het geeft de waarde _True_ indien het object is aangemaakt met de gegeven klasse, of met een van de ouderklassen van de gegeven klasse.

[NOTE]
--
Voor meer informatie over Object Georienteerd Programmeren in Perl 6, bekijk dan:
...
Getypeerde exceptions zijn object, daarom moeten we de `.new()` constructeur gebruiken in bovenstaand voorbeeld. +
...
=== Vergelijken van karaketers
Alphanumerieke karakters en het liggend streepje `_` mogen als zichzelf worden geschreven. +
Alle andere karakters moeten speciaal worden behandeld door er een backslash `\\` voor te plaatsen of te omgeven door aanhalingstekens.
...
=== Vergelijken Van Categorieen Van Karakters
Karakters kunnen worden geclassificeerd in categorieen en ook daarmee kunnen we vergelijken. +
We kunnen ook vergelijken de inverse van een categorie (alle karaketers behalve degene in die categorie).
...
Het vergelijken van categorieen van karakters zoals we in de vorige sectie zagen, is erg gemakkelijk. +
...
Het vraagteken `?` betekent nul of een keer.
...
Het plusteken `+` betekent tenminste een keer.
...
if 'Rakudo is een Perl 6 compiler' ~~ m/:s Perl 6/ {
    ...
    say "De string voor de gevonden string is: " ~ $/.prematch;
...
De string voor de gevonden string is: Rakudo is een
...
`.prematch` geeft de string voor de gevonden string terug. +
...
Indien een regex meer dan een witruimte bevat, dan is het gebruik van `:s` gemakkelijker in plaats van `\s` te specificeren voor elke witruimte.
...
.Uitvoer
`jan.mete@perl6.org is een gevalideerd emailadres`

.Uitleg
`<:L>` accepteert een enkele letter +
`<:L>+` accepteert een of meer letters +
`\.` accepteert een enkele [punt] +
`\@` accepteert een enkel @-karakter +
`<:L+:N>` accepteert een of meer letters gevolgd door een cijfer +
`<:L+:N>+` accepteert een of meer letters + cijfer een of meer keer +
...
Perl 6 maakt het mogelijk om *modules* aan te maken en die te distribueren.  Elke module is een bundel van functionaliteit die kan worden gebruikt zodra deze is geinstalleerd.

_Panda_ is een programma om modules te beheren dat deel is van Rakudo Star.
...
MD5 heeft een grote varieteit van applicaties waarvan het opslaan van wachtwoorden in een database er een is. 
...
Dit hoeft niet noodzakelijkerwijs een probleem te zijn tenzij dat je probeert te doen een hoop tijd vergt.

Natuurlijk heeft Perl 6 een aantal mogelijkheden die het mogelijk maken om delen van een programma in parallel uit te laten voeren. +
...
.Gelukkig kunnen we `is-prime` tegelijkertijd op meer dan een array element aanroepen:
[source,perl6]
----
my @array = (0..50000);                           #vullen van het Array
