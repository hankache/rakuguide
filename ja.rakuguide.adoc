...
* `shell` システムシェルを通じてコマンドを実行します。
  プラットフォームとシェルに依存します。
  すべてのシェルのメタ文字はシェルによって解釈されます。
  これには、パイプ、リダイレクト、環境変数の置換などが含まれます。
...
say map(-> $x {$x ** 2},@array);
----
平方を取るサブルーチンを宣言して `map` へ引数として渡すのではなく、 `\-> $x {$x ** 2}` のように無名サブルーチンで定義していることがわかります。
...
`.new()` メソッドに渡された引数は背後にあるオブジェクトの属性の設定に使われます。
...
NOTE: もし例外の型と、関連するメソッドのすべてのリストを知りたいのであれば、次のURLを参照することをすすめます https://docs.raku.org/type-exceptions.html
...
NOTE: Rakuのモジュールの一覧を見るには次のURLを参照してください: https://raku.land/

=== モジュールの使用
MD5は128ビットのハッシュ値を生成する暗号学的ハッシュ関数です。
MD5は、データベースに格納されているパスワードの暗号化などの様々な活用がされています。
...
say $hashed-password;
----
ハッシュを生成する `md5()` 関数を実行するために、この関数の実行に必要なモジュールをロードしなくてはなりません。 +
`use` キーワードはスクリプトの中で使いたいモジュールをロードし、そのモジュールから `md5` サブルーチンが提供されます。
...
`NativeCall` はRakuと一緒に提供されている標準モジュールで、RakuとCでやり取りする作業を簡単にする機能が色々と提供されています。
...
* link:https://stackoverflow.com/questions/tagged/raku[StackOverflow Raku questions] はRakuについてもっと詳しい回答が来る可能性がある場所です。

* link:https://rakudoweekly.blog[rakudoweekly] Rakudoとその周辺の出来事について今週のダイジェストをお伝えします。

* link:http://planet.raku.org[Planet Raku] Rakuのブログを集約しています。 Rakuにフォーカスしたブログ記事にこうご期待ください。

* link:https://www.reddit.com/r/rakulang/[/r/rakulang] Rakuのsubredditを購読しましょう。

* link:https://x.com/raku_news[@raku_news] Xでコミュニティをフォローしましょう。

* link:https://fosstodon.org/@rakulang[@rakulang] Mastodonでコミュニティをフォローしましょう。
