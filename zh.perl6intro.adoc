...
== Native Calling 接口 
Perl 6 可是让我们通过 Native Calling 借口来使用 C 库。
...
这个函数的功能是在终端中打印 `Hello from C`。它不接受参数，也不返回值。
...
其中 `$*CWD` 返回当前目录。 +
然后我们定义一个新的 Perl 6 子例程 `hellofromc` 作为在 `LIBPATH` 对应的 C 库中同名为 `hellofromc` 的 C 函数之包装函数。 +
这些通过使用 `is native` 特征来实现。 +
最后，我们调用我们的 Perl6 子例程。

总而言之，整个过程即声明一个带有 `is native` 和 C 库名特征的子程序。

=== 函数重命名

上面的例子中，我们看到了如何通过 `is native` 特征使用同名 Perl 6 子例程包装 C 函数来调用它。

在一些例子中，我们希望更改 Perl 6 子例程的名称。 +
...
.解释
由于 Perl 6 子例程与对应的 C 函数不同命，我们在这里使用 `is symbol` 来提供原始的 C 函数名。

=== Passing Arguments

编译下面更改过的的 C 库代码并运行下面的 Perl 6 脚本。 +
注意我们是如何修改 C 和 Perl 6 代码接受一个字符串参数（在 C 中是 `chr*`，在 Perl 6 中是 `Str`） 
...
让我们再来定义一个简单的计算器，实现接受两个整数输入并返回它们之和。 +
...
NOTE: 更多关于 Native Calling 借口, 详见 https://docs.perl6.org/language/nativecall
