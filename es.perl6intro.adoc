...
El número y tipo de argumentos que acepta una subrutina se denomina su *sobrecarga*.
...
Es posible definir varias subrutinas con el mismo nombre pero con distintas sobrecargas.
...
Para restringir el valor de retorno a un tipo determinado podemos utilizar `returns` o la notación de flecha `-\->` en la sobrecarga.
...
Al igual que las subrutinas, los métodos pueden empaquetar un conjunto de funcionalidades, aceptar *argumentos*, tener una *sobrecargas* y definirse como *múltiples*.
