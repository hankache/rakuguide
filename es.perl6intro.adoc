...
say 'a' cmp 'B'; # Más
----
Este ejemplo dice que `a` es mayor a `B` porque el número de código de `a` es mayor que el número de código de `B`.

Si bien la lógica anterior es técnicamente correcta, puede que no sea muy útil.
...
Uno de estos operadores es `unicmp`, funciona como el que hemos visto `cmp` pero contemplado por Unicode.
[source,perl6]
----
