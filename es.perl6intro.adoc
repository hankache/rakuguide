...
Dependiendo del tipo se pueden realizar una serie de operaciones concretas.
...
NOTE: Consulta https://docs.perl6.org/type/Str para ver la lista completa de métodos disponibles de texto.
...
NOTE: Consulta https://docs.perl6.org/type/Int para ver la lista completa de métodos disponibles de enteros.
...
NOTE: Consulta https://docs.perl6.org/type/Rat para ver la lista completa de métodos disponibles de números racionales.
...
Se pueden realizar muchas operaciones con arrays:
...
`proceed` continúa la evaluación aunque se produzca la coincidencia.
...
El flujo de las llamadas a los métodos es de abajo hacia arriba, esto es, el último paso precedido por los pasos anteriores y en orden.
...
Lo que hace el twigil `.` es declarar un atributo con `!` y crear automáticamente el método de acceso.
