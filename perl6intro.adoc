...
Depending on the type, different operations can be performed.
...
NOTE: For the complete list of available String methods, see https://docs.perl6.org/type/Str
...
NOTE: For the complete list of available Integer methods, see https://docs.perl6.org/type/Int
...
NOTE: For the complete list of available Rational Number methods, see https://docs.perl6.org/type/Rat
...
Many operations can be performed with arrays:
...
`proceed` will instruct Perl 6 to continue matching even after a successful match.
[source,perl6]
----
my $var = 42;
...
`map` is a *higher order function*, it cam accept another function as an argument.
...
The flow of the method calls is bottom-up -- final step, preceded by previous steps, in order.
...
What the `.` twigil does is declare an attribute with `!` and automate the creation of an accessor.
...
* `.^attributes` -- returns all the attributes for the object
