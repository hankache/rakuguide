...
== Loops and conditions (pengulangan dan kondisi)
...
If you run the above code, it will display `Good Morning Wiro Sableng` on the terminal. +
...
In this case, `$generated` is a closure that incorporates both the `greeting` subroutine and the `Wiro Sableng` string that existed when the closure was created.
