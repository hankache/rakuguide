...
Questo array potrà tenere al massimo tre volri, indicizzati da 0 a 2.
...
Otterresti il seguente messaggio di errore: [l'indice 3 è fuori di uno dall'intervallo consentito (che deve essere 0..2) ndt.]
----
...
Fallirà con questo messaggio di errore: `Type check failed in assignment to $var; expected Int but got Str` [ossia `controllo sul tipo fallito per $var; mi aspettavo un Int ma ho ricevuto Str`]
...
| *Tipo* | *Descrizione* | *Example* | *Result*
