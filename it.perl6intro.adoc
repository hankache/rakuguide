...
.Explanation
The `grafico-combinato` class should be able to hold two series, one for the actual values plotted on bars,
and another for forecast values plotted on a line. +
This is why we defined it as a child of `grafico-a-linee` and `grafico-a-barre`. +
You should have noticed that calling the method `traccia` on the `grafico-combinato` didn't yield the required result.
Only one series was plotted. +
Why did this happen? +
`grafico-combinato` inherits from `grafico-a-linee` and `grafico-a-barre`, and both of them have a method called `plot`.
When we call that method on `grafico-combinato` Perl 6 internals will try to resolve the conflict by calling one of the inherited methods.

.Correction
In order to behave correctly, we should have overridden the method `traccia` in the `grafico-combinato`.
...
the one where we _forgot_ to override the `traccia` method.
...
class umano {
  ...
  has Int $.anni;
  method presentati {
    say 'Hi I am a human being, my nome is ' ~ self.nome;
  }
}
...
class impiegato is umano {
  ...
  has Int $.salario;
  method presentati {
    say 'Hi I am a employee, my nome is ' ~ self.nome ~ ' and I work at: ' ~ self.azienda;
  }
}
...
say $gianna.^methods;
say $gianna.^parents;
if $gianna ~~ umano {say 'Gianna is a umano'};
----
...
try {
  ...
  CATCH {
    default {
      say "Can you tell us your nome again, we couldn't find it in the register.";
    }
  }
}
say "How are you doing today?";
...
.`Output`
----
Can you tell us your nome again, we couldn't find it in the register.
How are you doing today?
----
...
----
if "Giovanni123" ~~ / \d / {
  say "This is not a valid nome, numbers are not allowed";
} else {
  say "This is a valid nome"
}
if "Giovanni-Doe" ~~ / \s / {
...
first nome [dot] last nome [at] azienda [dot] (com/org/net)
