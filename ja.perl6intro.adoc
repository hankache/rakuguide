...
* もし文字列がアポストロフィを含んでいるなら

* もし文字列が展開される必要のある変数を含んでいるなら
...
| ! | 接頭辞 | 被演算子をブーリアン値にし、その否定を返す | !4 | False
...
.2+| {vbar} .2+| 接頭辞 .2+| Flattening | {vbar}(0..5)  | (0 1 2 3 4 5)

<| {vbar}(0\^..^5)  <| (1 2 3 4)

|===

注釈: もしその演算の優先順位を含んだ、演算子のすべてのリストを知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/language/operators

== 変数
Perl6の変数は三つのカテゴリに分類される: スカラ、配列、ハッシュです。
...
注釈: もし文字列に対して適用できるすべてのメソッドのリストを知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/type/Str
...
注釈: もし整数に対して適用できるすべてのメソッドのリストを知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/type/Int
...
注釈: もし有理数に対して適用できるすべてのメソッドのリストを知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/type/Rat
...
注釈: もし配列に関するすべての情報を知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/type/Array
...
注釈: もしハッシュに関するすべての情報を知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/type/Hash
...
変数は任意の型の値を持つことができるという意味で動的なのです。
...
注釈: もし変数に関する情報をもっと知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/type/variables

== 関数とミューテーター

関数とミューテーターを区別することは重要です。 +
関数はそれが呼ばれたときのオブジェクトの状態を変更しません。+
ミューテーターはオブジェクトの状態を変更します。
...
関数にミューテーターとしてふるまうように強制するために`.`のかわりに`.=`を使っています。(#4) (スクリプトの9行目)
...
別の選択肢として、`proceed` を使うと、Perl6は条件が満たされた後も処理の実行を続けます。
[source,perl6]
----
my $var = 42;
...
注釈: もしループ構文と条件文に関する情報をもっと知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/language/control
...
警告: Windowsのユーザーはディレクトリを定義するために `/` か `\\` を使うことができます +
`C:\\rakudo\\bin` +
`C:/rakudo/bin` +

注釈: もしI/Oに関する情報をもっと知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/type/IO
...
また、サブルーチンが引数として受け取る数や型は*シグネチャ*と呼ばれます。
...
サブルーチンが呼ばれると、ランタイム環境は引数として与えられた数と型をもとにしてどのバージョンを使うべきであるか決定します。
...
注釈: もしサブルーチンと関数に関する情報をもっと知りたいのであれば、次のURLを参照することをすすめます http://doc.perl6.org/language/functions
...
.説明
まず`squared`と呼ばれるサブルーチンを定義しました。このサブルーチンは引数として与えられた数を二乗します。
...
=== クロージャ
Perl6のすべてのコードオブジェクトはクロージャです。これは外のスコープのレキシカル変数を参照できるということを意味しています。
...
Perl6ではこのような無名関数の使い方を*ポインティブロック*と呼びます。
...
上記の例とは対照的に、Perl6ではメソッドのチェーンを行うことができます。+
上記の例は*メソッドチェーン*を利用して、次のように書くことができます。
...
メソッドチェーンは _見た目が良い_ ということが一目瞭然ですね。

=== フィード演算子
*フィード演算子* は、関数型言語では _パイプ_ と呼ばれ、メソッドチェーンをさらに見やすくしてくれます。
[source,perl6]
.Forward Feed
----
my @array = <7 8 9 0 1 2 4 3 5 6 7 8 9>;
...
クラスは`my`を使うことで_レキシカルスコープ_とすることもできます:
[source,perl6]
----
my class Human {
...
デフォルトでは、すべての属性はprivateですが、いつも`!`トゥイジルを使うことはよい習慣です。
...
`is rw`キーワードを使って、_読み/書き_ラベルを付与する必要があります。
[source,perl6]
----
class Human {
  has $.name;
...
注釈: もし例外の型と、関連するメソッドのすべてのリストを知りたいのであれば、次のURLを参照し、Xから始まる型のところへ進むのをすすめます http://doc.perl6.org/type.html
...
_マッチオブジェクト_から次のようなメソッドを呼ぶことができます: +
...
別の選択肢としては`m/ Perl\s6 /` と書くこともできます。初めに触れたように `\s` は空白文字のためのプレースホルダです。+
...
警告: この例で使われている正規表現はあまり正確ではありません。+
...
注釈: もしもっと正規表現について知りたいのであれば、次のURLを参照するのをすすめます http://doc.perl6.org/language/regexes
...
注釈: Perl6のモジュールの一覧を見るには次のURLを参照してください: http://modules.perl6.org/
...
say $hashed-password;
----
ハッシュを生成する `md5_hex()` 関数を実行するために、要求されたモジュールをロードしなくてはなりません。+
`use`キーワードはスクリプトの中で使いたいモジュールをロードします。

警告: 実用的にはMD5ハッシュ単独では不十分です、なぜなら辞書攻撃を受けやすいからです。+
...
現在のところ、二つの事柄のうちの一つを意味するということを頭にとどめておくことが重要です:
...
もしコードが正しく評価されたのなら_約束_は*守られ*ているでしょう。+
もしコードが例外を投げたのなら_約束_は*破られ*ているでしょう。
...
警告: 並列処理にはスレッディングのオーバーヘッドがあります。もしオーバーヘッドが計算速度で相殺されないのなら、スクリプトが遅くなってしまったように見えるでしょう。+
これが`race`、 `hyper`、 `start`、 `await` をいたってシンプルなスクリプトに対して用いると実際には遅くなってしまう理由です。

=== 平行性と非同期性
注釈: 並行/非同期プログラミングについてもっと情報を知りたいのなら、次のURLを参照してください: http://doc.perl6.org/language/concurrency
