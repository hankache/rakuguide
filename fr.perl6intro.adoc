...
Les lignes de code uniques peuvent aussi être tapées de façon non interactive sur la ligne de commandes en tapant `perl6 -e 'mon code ici'` et ensuite [Entrée].
...
=== Scalaire
Un scalaire contient une valeur ou un référence.
...
NOTE: Pour une liste exhaustive des méthodes applicables aux Entiers, see http://doc.perl6.org/type/Int
...
NOTE: Pour une liste exhaustive des méthodes applicables aux Nombres rationnels, see http://doc.perl6.org/type/Rat
...
Le tableau simple à une taille non définie, et peut s'auto indexer. +
...
Lorsque la routine est appelé, l'environnement d'exécution décidera quelle version utiliser en fonction du nombre et du type des arguments fournis.
...
La routine anonyme `\-> $x {$x ** 2}` n'a pas de handle et ne peut donc pas être appelée.
