...
Un routine est défini avec le mot-clé `sub`. Après leur définition, ils peuvent être appelés par leur nom. +
...
L'exemple précédent présente un routine qui ne nécessite aucun argument.

=== Signature
Beaucoup de routines requièrent une certaine entrée pour fonctionner. Cette entrée est fournie par des *arguments*. +
La *signature* est le nombre et le type d'arguments que le routine accepte.

Le routine ci-dessous accepte une chaîne pour argument:
...
Lorsque le routine est appelé, l'environnement d'exécution décidera quelle version utiliser en fonction du nombre et du type des arguments fournis.
Ce type de routines est défini de la même manière que les routines normaux sauf que nous utilisons le mot-clé 'multi' a la place de 'sub'.
...
Si un routine est défini comme acceptant un argument, et nous l'appelons sans fournir l'argument requis, le routine va échouer.
...
Si l'utilisateur ne fournit pas un argument, le routine peut fournir une valeur par défaut. +
Cela se fait par l'attribution d'une valeur à l'argument durant la définition du routine.
...
Ce chapitre traitera de des fonctionnalités facilitant la programmation fonctionnelle
...
.Explication
Nous avons définit un routine appelé `carré`, qui met à la puissance 2 tous les arguments qui lui sont passés. +
Ensuite, nous avons utilisé `map`, un fonction d'ordre supérieur en lui passant 2 arguments, un routine et un tableau.+
Le résultat est une liste de tous les éléments du tableau mis à la puissance 2.

Notez que quand un routine est passé comme argument, nous le préfixons avec `&`.
...
----
Notez qu'au lieu de déclarer un routine et de le passer en argument à `map`, nous le déclarons directement à l'intérieur de `map`. +
Le routine anonyme `\-> $x {$x ** 2}` n'a pas de handle et ne peut donc pas être appelé.
...
Nous pouvons également définir de nouveaux routines et les appeler en utilisant l'hyperopérateur. En ce cas, il faut préfixer la méthode avec `&`. ex: `&est-pair` 
