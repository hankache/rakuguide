...
NOTA: Para a lista completa de operadores, incluindo sua precedência, visite https://docs.perl6.org/language/operators
...
NOTA: Para a lista completa de métodos aplicáveis a Strings, veja https://docs.perl6.org/type/Str
...
NOTA: Para a lista completa de métodos aplicáveis a Inteiros, visite https://docs.perl6.org/type/Int
...
NOTA: Para a lista completa de métodos aplicáveis a Números Racionais, visite http://doc.perl6.org/type/Rat
...
DICA: O til `~` é usado para concatenar strings.
...
NOTA: Para uma referência completa sobre Arrays, veja https://docs.perl6.org/type/Array
...
NOTA: Para uma referência completa sobre Hashes, veja https://docs.perl6.org/type/Hash

=== Tipos
Nos exemplos anteriores, nós não especificamos o tipo dos valores que as variáveis devem conter.

DICA: `.WHAT` retorna o tipo do valor contido em uma variável.
...
NOTA: Para mais informações sobre variáveis, veja https://docs.perl6.org/language/variables
...
NOTA: Para mais informações sobre laços e condicionais, veja https://docs.perl6.org/language/control
...
ATENÇÃO: Usuários de Windows podem usar `/` ou `\\` para definir diretórios. +
`C:\\rakudo\\bin` +
`C:/rakudo/bin` +

NOTA: Para mais informação sobre I/O, veja https://docs.perl6.org/type/IO
...
NOTA: Para mais informações sobre sub-rotinas e funções, veja http://doc.perl6.org/language/functions
...
NOTA: Para uma lista completa de tipos de exceção e seus métodos associados visite https://docs.perl6.org/type-exceptions.html
...
DICA: Por padrão os espaços em branco numa definição de regex são irrelevantes. +
...
ATENÇÃO: a regex usada nesse exemplo não é muito precisa para validação de emails. +
...
NOTA: Para mais informações sobre regexes, veja https://docs.perl6.org/language/regexes
...
NOTA: O diretório de módulos Perl 6 pode ser encontrado em: https://modules.perl6.org/
...
ATENÇÃO: O uso de hashing MD5 não é suficiente, porque é um método vulnerável a ataques de dicionário. +
...
ATENÇÃO: Paralelismo sempre adiciona um custo (overhead) de threading. Se esse overhead não for compensado pelo ganho em velocidade computacional, o script pode parecer mais lento. +
...
NOTA: Para mais informações sobre Concorrência e Programação Assíncrona, veja https://docs.perl6.org/language/concurrency
